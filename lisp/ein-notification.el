...
(defun ein:header-line-select-window (key-event)
  (ein:with-destructuring-bind-key-event key-event
    (ignore width dx image col text-pos object timestamp x pos-or-area event-type)
    (select-window window)))

(defun ein:header-line-key-event-get-worksheet (key-event)
  (ein:with-destructuring-bind-key-event key-event
    (ignore window width dx image col text-pos timestamp x pos-or-area event-type)
