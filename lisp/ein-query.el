...
(defun ein:query-prepare-header (url settings &optional securep)
  ...
  (let* ((host (url-host (url-generic-parse-url url)))
         (path (let ((warning-minimum-level :emergency)
                     (warning-minimum-log-level :emergency))
                 (when-let ((root-url (car (ein:notebooklist-parse-nbpath url)))
                            (root-path (url-filename (url-generic-parse-url root-url))))
                   (unless (zerop (length root-path))
                     (file-name-as-directory root-path)))))
         (cookies (or (when (stringp path)
