...
(cl-defmethod ein:cell-to-nb4-json ((cell ein:codecell) wsidx &optional discard-output)
  (let* ((ss-table (ein:get-slide-show cell))
         ...
         (execute-count (ein:aif (ein:oref-safe cell :input-prompt-number)
                            (and (numberp it) it))))
    (plist-put metadata :collapsed (if (slot-value cell 'collapsed) t json-false))
    (plist-put metadata :autoscroll json-false)
    (plist-put metadata :ein.tags (format "worksheet-%s" wsidx))
    (plist-put metadata :slideshow ss-table)
